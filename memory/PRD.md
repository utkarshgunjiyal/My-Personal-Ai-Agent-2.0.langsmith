...
- [x] **DONE 2026-06-22 — LangSmith Level-1 tracing**: `@traceable` on `call_llm` / `stream_llm` (with `process_inputs`/`process_outputs` so LLM spans render in OpenAI chat-completion schema; `reduce_fn` aggregator for streamed tokens) / `_run_one_agent` / `_score_trace`. Each `/api/ask/stream` is wrapped as a single root `ask_stream` trace via `tracing.trace_root` (graceful no-op when `LANGSMITH_TRACING` is unset). Judge scores (per-agent + `judge_best_score`) are pushed to the LangSmith Feedback API. Root run carries `final_answer` outputs + metadata (user_id, thread_id, best_agent, has_uploads, upload_count) + tags (`agent:<name>`, `judge:<name>`, `cache_hit`, `has_uploads`). `ls_run_id` + `ls_url` are emitted in the SSE `done` event AND persisted on the assistant message doc, so each message in the UI deep-links to its trace via a `trace ↗` button. Smoke test `backend/langsmith_smoke.py` verifies the full flow end-to-end. Project: `ai-mentor-prod`.
